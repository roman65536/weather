...
//$query="select timestamp 'epoch' + interval '1 second'* (round(extract('epoch' from datetime) / 300) *300), avg(in1_v), avg(in2_v), avg(out_v) from in_out where datetime >= '2017-08-06' group by  round(extract('epoch' from datetime) / 300) order by 1;"


$query  = "select timestamp 'epoch' + interval '1 second'* (round(extract('epoch' from datetime) / 300) *300),avg(out_v) as out_v,avg(in1_v) as in1_v,avg(in2_v) as in2_v from ".$_REQUEST['table']." ";
//$query  = "select * from ".$_REQUEST['table']." ";
$query .= "where datetime>='".$_REQUEST['from']."' and  datetime<='".$_REQUEST['to']."'";
//$query .= " and sensor='".$_REQUEST['sys']."' order by datetime";
$query .= " group by round(extract('epoch' from datetime) / 300) order by 1";
...
$fields=array();
$tmp=pg_query($con,$query);
$i = pg_num_fields($tmp);
for($j=0;$j< $i; $j++) {
 $tmp_name=pg_field_name($tmp,$j);
 fwrite($tmpfile, $tmp_name ."    ");
 array_push($fields,$tmp_name);
}
fwrite($tmpfile,"\n#\n");
...
while ( $x < $z)
{
 $res=pg_fetch_row($tmp,$x);
 for($j=0;$j<$i;$j++)
 {
  if ($res[$j] == "")
   fwrite($tmpfile, "?   ");
   else
  fwrite($tmpfile, $res[$j] . "    ");
 }
 fwrite($tmpfile,"\n");
 $x++;
}
...
$proc = proc_open("/usr/bin/tee /tmp/tee1.out | /usr/bin/gnuplot", $desc, $pipes);
if (!is_resource($proc)) {
  ...
  fwrite($pipes[0], "set terminal png truecolor small size 640,480\n");

  // set X axe
  fwrite($pipes[0], "set xdata time\n");
  fwrite($pipes[0], "set timefmt \"%Y-%m-%d %H:%M:%S\"\n");
  fwrite($pipes[0], "set grid\n");
  fwrite($pipes[0], "set offset 0,0,.5,.5\n");
  //fwrite($pipes[0], "set xtics rotate by 45 right\n");
 ...
 if($title)
  ...
  $range = array();
  foreach ($fields as $k => $v) {  array_push ($range,"$".($k+2)); fprintf($dbg,"1: %s\n", $k); }
