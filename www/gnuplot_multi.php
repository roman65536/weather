...
$query  = "select datetime,avg(out_v) as out_v,avg(in1_v) as in1_v,avg(in2_v) as in2_v from ".$_REQUEST['table']." ";
...
$query .= " group by UNIX_TIMESTAMP(datetime) DIV 30 order by datetime";
...
if (!is_resource($proc)) {
 ...
 if($title)
  ...
  $range = array();
  foreach ($fields as $k => $v)   array_push ($range,"$".($k+2));
