...
<?php
 ...
 if ((!array_key_exists('date_von',$_REQUEST)) or
     (!array_key_exists('date_bis', $_REQUEST)) )
   {
     ...
     $date_von=date('d-m-Y', strtotime('-1 week'));
   }
 else
  {
...
$query="select sensor from sensors  where datetime >= '".$from."' AND datetime <= '".$to."' group by 1"; 
...
$i=0;
while ($i < $num) {
$sensor=mysql_result($result,$i,"sensor");

?>

<tr>
<td><font face="Arial, Helvetica, sans-serif"><?php echo "$sensor"; ?></font></td>
...
while ($i < $num) {
$sensor=mysql_result($result,$i,"sensor");
	$params  ="?sys=".$sensor."&from=".$from."&to=".$to;
	$params .="&table=sensors";
	$params .="&title=Sensor Data ".$sensor;
	$params .="&graphs=value";
	$params .="&graphs_names=values";
	print "<img src=\"gnuplot_temp.php".str_replace(array(" ","\"","+"),array("%20","%22","%2B"),$params)."\"><br>\n";
...
$params  ="?sys=".$sensor."&from=".$from."&to=".$to;
        ...
        print "<img src=\"gnuplot_multi.php".str_replace(array(" ","\"","+"),array("%20","%22","%2B"),$params)."\"><br>\n";
